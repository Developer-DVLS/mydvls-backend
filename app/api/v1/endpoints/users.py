...
from app.api.v1.schemas.users import ChangePassword, ForgotPassword, ResetPasswordConfirmation, UserLoginResponseSchema, UserRegisterRequest, UserResponse, UserVerified
...
async def user_register(background_tasks: BackgroundTasks, data:UserRegisterRequest , db:  AsyncSession = Depends(get_db)):
    ...
    try:
        ...
        await send_email(background_tasks=background_tasks,
                    ...
                    context={'user': new_user.first_name + " " + new_user.first_name,
                            'verification_code': verification_token,
                            'verification_link': f'{settings.FRONTEND_URL}/activation/{verification_token}'
                    }
                    )
